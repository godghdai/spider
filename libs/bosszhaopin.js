	...
	return new Promise(function (resolve, reject) {
		...
		async.times(pagenum, function (n, nextPage) {
			...
			async.retry(retrytime, function (retry) {
				...
				request.get(`https://www.zhipin.com/c100010000/h_101010100/?query=Node.js&page=${n + 1}&ka=page-${n + 1}`)
					.end(function (err, res) {
						...
						$(".job-list ul li").each(function (i, e) {
							var jobprimary = $(e).find(".job-primary"),
								...
								infoprimaryp = jobprimary.find(".info-primary p")
									.contents()
									.toArray()
									.filter(item => {
										return item.type == "text";
									}),
								companytextp = infocompany.find(".company-text p")
									.contents()
									.toArray()
									.filter(item => {
										return item.type == "text";
