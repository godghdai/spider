...
var getUrl = function (page, city = "全国", kd = "node.js") {
	return `https://www.lagou.com/jobs/positionAjax.json?px=default&needAddtionalResult=false&city=${urlencode(city)}&first=${page == 1 ? true : false}&kd=${urlencode(kd)}&pn=${page}`;
}
			...
			savePhoto: ['makeImgDir', 'getData', function (results, callback) {
				...
				async.eachLimit(results["getData"], 5, function (item, next) {
					var imgurl = item.companyLogo,
						imgpath = "no",
						companyShortName;
					//"i/image/M00/1F/43/CgpFT1kRms6ALcwgAAAVva3NK58840.jpg"  
					//"image1/M00/3F/D5/CgYXBlXBtseABpXHAAAOGSmg5jM209.gif?cc=0.6404429662507027"
					/([^\/]*\.(jpg|gif|png))/.test(imgurl) && (imgpath = RegExp.$1);
					console.log(imgpath);
					//path.posix.basename(imgurl)
					request.get('https://www.lgstatic.com/thumbnail_120x120/' + imgurl)
						.pipe(fs.createWriteStream(path.join(imgDir, imgpath)))
						.on('close', function () {
